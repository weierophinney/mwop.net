---
id: 213-From-the-inside-out-How-to-layer-decorators
author: matthew
title: 'From the inside-out: How to layer decorators'
draft: false
public: true
created: '2009-04-06T08:30:00-04:00'
updated: '2009-04-12T09:41:00-04:00'
tags:
    0: php
    2: 'zend framework'
---
...
You may have noticed in the [previous installment](/blog/212-The-simplest-Zend_Form-decorator.html) that the decorator's `render()` method takes a single argument, `$content`. This is expected to be a string. `render()` will then take this string and decide to either replace it, append to it, or prepend it. This allows you to have a chain of decorators — which allows you to create decorators that render only a subset of the element's metadata, and then layer these decorators to build the full markup for the element.
...
- The input is then passed as content to the SimpleLabel decorator, which generates a label and prepends it to the original content; the default separator is a `PHP_EOL` character, giving us this: `<label for="bar-foo"> <input id="bar-foo" name="bar[foo]" type="text" value="test"/>`
...
The above results in the markup `<input id="bar-foo" name="bar[foo]" type="text" value="test"/> <label for="bar-foo">`.
