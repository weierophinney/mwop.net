 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Libmemcached extends AbstractBackend implements ExtendedBackend
{
    /**
    ...
    public function __construct(array $options = array())
    {
        if (!extension_loaded('memcached')) {
            Cache\Cache::throwException('The memcached extension must be loaded for using this backend !');
        }
    ...
    public function clean($mode = Cache\Cache::CLEANING_MODE_ALL, $tags = array())
    {
        switch ($mode) {
            ...
            case Cache\Cache::CLEANING_MODE_OLD:
                $this->_log(__METHOD__ . "() : CLEANING_MODE_OLD is unsupported by the Libmemcached backend");
                break;
    ...
    {
        $this->_log(__METHOD__ . "() : getting the list of cache ids is unsupported by the Libmemcached backend");
        return array();
    }
        ...
        if (isset($tmp[0], $tmp[1], $tmp[2])) {
            ...
            if (!($result = $this->_memcache->replace($id, array($data, time(), $newLifetime), $newLifetime))) {
                ...
                if ($result === false) {
                    $rsCode = $this->_memcache->getResultCode();
                    $rsMsg  = $this->_memcache->getResultMessage();
                    $this->_log(__METHOD__ . "() failed: [{$rsCode}] {$rsMsg}");
                }
            }
            return $result;
        }
